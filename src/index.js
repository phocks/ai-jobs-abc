...
require("./index.scss").default;
const template = require("./template");

// Declare a few variables
let anzscoLookup, fuse;
...
const main = async () => {
  ...
  const complete = new autoComplete({
    ...
    source: function (term, suggest) {
      ...
      let suggestionCount =
        fuseResult.length > maxResults
          ? (suggestionCount = maxResults)
          : fuseResult.length;
  ...
  function selectGroup(selectedGroupData, jobTitle) {
    ...
    if (app.percentMoreSusceptible <= 25) app.reactionMessage = "Phew, only";
    else if (
      app.percentMoreSusceptible > 25 &&
      app.percentMoreSusceptible <= 60
    )
      app.reactionMessage = "Hmm,";
    else if (
      app.percentMoreSusceptible > 60 &&
      app.percentMoreSusceptible <= 80
    )
      app.reactionMessage = "Gosh,";
    else if (app.percentMoreSusceptible > 80) app.reactionMessage = "Whoa,";

    // Set comparison message to display
    if (selectedGroupData.percentMoreSusceptible <= 35)
      app.comparisonMessage = "less susceptible to";
    else if (
      selectedGroupData.percentMoreSusceptible > 35 &&
      selectedGroupData.percentMoreSusceptible <= 65
    )
    ...
    app.shareText = encodeURIComponent(
      selectedGroupData.percentMoreSusceptible +
        "% of my job is susceptible to automation. Could a robot do your job? Find out here"
    );
      ...
      drawWaffle(percent, section, text) {
        ...
        const circles = waffleGroup
          ...
          .attr("r", function (d) {
            switch (section) {
              case "less":
                if (d.label === "more") return unitSize - 0.5;
                else return unitSize;
                break;
              case "more":
                if (d.label === "less") return unitSize - 0.5;
                else return unitSize;
            }
          })
          ...
          .attr("stroke", function (d) {
            switch (section) {
              case "less":
                if (d.label == "more") return "rgba(0, 0, 0, 0.3)";
                else return "rgba(0, 0, 0, 0.0)";
                break;
              case "more":
                if (d.label == "less") return "rgba(0, 0, 0, 0.3)";
                else return "rgba(0, 0, 0, 0.0)";
            }
          })
        ...
        const percentText = svg
          ...
          .style("dominant-baseline", "alphabetical")
          .text(function () {
            if (section === "more") return percent;
  ...
  Vue.component("barcode-chart", {
    ...
    methods: {
      drawBarcode(yourJobPercent, highlightPercent) {
        ...
        const data = jobs.automationData.sort(function (a, b) {
        ...
        const chartScale = d3.scaleLinear().domain([0, 100]).range([0, 100]); // fallback to percentage as x indicator
        ...
        function drawChart(d3El, data, highlightPosition, yourBarPosition) {
          ...
          var highlightBar = barcodeGroup
            ...
            .attr(
              ...
              "translate(0, " +
                "-" +
                (highlightBarHeight - chartHeight) / 2 +
                ")"
            )
  ...
  function renderYourBarToComparison(yourJobPercent) {
    ...
    barcodeGroup
      ...
      .attr(
        "transform",
        "translate(0, " + "-" + (highlightBarHeight - chartHeight) / 2 + ")"
      )
      .style("fill", yourBarColor)
      .attr("x", function (d) {
        return Math.floor(chartScale(yourJobPercent)) + "%";
  ...
  const jobsInList = automationList
    .selectAll("div")
    .data(data)
    .enter()
    .append("h3")
  ...
  barcodeGroup
    .append("rect")
    .attr("width", (d) => {
      return chartScale(d.percentMoreSusceptible) + "%";
    })
    .attr("height", chartHeight)
    .style("fill", "rgba(255, 155, 0, 0.15)");
  ...
  barcodeGroup
    ...
    .attr("dominant-baseline", "alphabetical")
    .text((d) => {
      return d.percentMoreSusceptible + "%";
    });
  ...
  function reorder(sortOrder) {
    d3.selectAll("h3.job-group-title").sort(function (a, b) {
      switch (sortOrder) {
        case "ascending":
          // Prevent unpredictable behaviour when values are identical
          if (a.percentLessSusceptible !== b.percentLessSusceptible)
            return d3.ascending(
              a.percentLessSusceptible,
              b.percentLessSusceptible
            );
          else return a.groupTitle.localeCompare(b.groupTitle);
          break;
        case "descending":
          if (a.percentLessSusceptible !== b.percentLessSusceptible)
